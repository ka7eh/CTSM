module SoilBiogeochemCarbonStateType
  ...
  type, public :: soilbiogeochem_carbonstate_type
     ...
     real(r8), pointer :: decomp_cpools_vr_col (:,:,:) ! (gC/m3) vertically-resolved decomposing (litter, cwd, soil, doc) c pools
...
contains
  ...
  subroutine Summary(this, bounds, num_allc, filter_allc)
    ...
    ! Add doc carbon pools together to produce vertically-resolved decomposing total doc pool
    if ( nlevdecomp_full > 1 ) then
      do j = 1, nlevdecomp
         do fc = 1,num_allc
            c = filter_allc(fc)
